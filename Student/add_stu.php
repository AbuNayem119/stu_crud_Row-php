...
<?php
    ...
    if (isset($_POST['submit'])){
        ...
        $final_img_name = md5(time().rand().$image).".".$array_end_part;
        ...
        if (empty($name)||empty($email)||empty($cell)||empty($address)||empty($image)) {
            ...
            $sql = "INSERT INTO student (name, email, cell, address, image) VALUES ('$name','$email','$cell','$address','$final_img_name')";
            $conn -> query($sql);

            move_uploaded_file($tmp_img, "image/".$final_img_name);

            $mess = "<p class='alert alert-success'> Data sent successfully !<button style='float:right;' class='close' data-bs-dismiss='alert' >&times;</button></p>";
...
?>
    ...
    <div class="mt-2 w-50 mx-auto card">
        ...
        <div class="card-footer">
            <a style="display: block;" class="btn btn-info" href="../dashboard.php">Back to Dashboard</a>
        </div>
    </div>
